...
class FirebaseWebRtcService implements WebRtcService {
...
let firebaseWebRtcService: WebRtcService | null = null;

export function getFirebaseWebRtcService(): WebRtcService {
  if (runtimeMode !== "firebase") {
    throw new Error("Firebase WebRTC transport is unavailable in local mode.");
  }
  firebaseWebRtcService ??= new FirebaseWebRtcService();
  return firebaseWebRtcService;
}

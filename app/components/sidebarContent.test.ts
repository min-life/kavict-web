...
describe("sidebar content", () => {
  it("defines Practice Space, static task progress, and a leaderboard destination", async () => {
    expect(existsSync(contentModulePath)).toBe(true);

    const sidebarContent = await import("./sidebarContent");
    const { LEADERBOARD_NAV_ITEM, PRIMARY_NAV_ITEMS, STATIC_TASKS } = sidebarContent;

...
export default function KaviAdvisorTab({ userId, plan, transactions, onPlanApplied }: KaviAdvisorTabProps) {
  const [messages, setMessages] = useState<AdvisorMessage[]>([{ sender: "assistant", text: "Chào bạn, mình là Kavi Advisor. Hãy kể cho mình về thu nhập, chi tiêu và mục tiêu hiện tại nhé." }]);
  ...
  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
      <section className="flex min-h-[38rem] flex-col overflow-hidden rounded-2xl border border-outline-variant/30 bg-surface shadow-sm">
        <header className="flex items-center gap-3 border-b border-outline-variant/20 bg-primary/5 p-5">
          <span className="material-symbols-outlined rounded-full bg-primary p-2 text-on-primary">smart_toy</span>
          <div>
            <h2 className="font-bold text-on-surface">Kavi Advisor</h2>
            <p className="text-xs text-on-surface-variant">{isFallbackMode ? "Chế độ local – phản hồi mẫu" : "Tư vấn theo dữ liệu kế hoạch hiện tại"}</p>
          </div>
        </header>
        <div className="flex-1 space-y-4 overflow-y-auto p-5">
          {messages.map((message, index) => <div key={`${message.sender}-${index}`} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${message.sender === "user" ? "rounded-br-sm bg-primary text-on-primary" : "rounded-bl-sm border border-outline-variant/30 bg-surface-container-lowest text-on-surface"}`}><ReactMarkdown>{message.text}</ReactMarkdown></div></div>)}
          {isSending && <div className="text-sm text-on-surface-variant">Kavi đang suy nghĩ...</div>}
          <div ref={chatEnd} />
        </div>
        <form onSubmit={send} className="flex gap-2 border-t border-outline-variant/20 p-4">
          <input value={input} onChange={(event) => setInput(event.target.value)} placeholder={isFallbackMode ? "Nhập Yes để tạo sample" : "Mô tả tình hình tài chính của bạn..."} className="min-w-0 flex-1 rounded-xl border border-outline-variant/50 bg-surface px-3 py-2.5 text-on-surface outline-none focus:border-primary" />
          <button disabled={isSending} className="rounded-xl bg-primary px-4 py-2 text-on-primary disabled:opacity-50"><span className="material-symbols-outlined">send</span></button>
        </form>
      </section>
      <aside className="rounded-2xl border border-outline-variant/30 bg-surface p-5 shadow-sm">
        <h3 className="font-bold text-on-surface">Plan được đề xuất</h3>
        {proposal ? <><p className="mt-3 text-sm text-on-surface-variant">Thu nhập tháng: <strong className="text-on-surface">{new Intl.NumberFormat("vi-VN").format(proposal.monthlyIncome)} ₫</strong></p><p className="mt-2 text-sm text-on-surface-variant">{proposal.budgets.length} danh mục ngân sách · {proposal.goals.length} mục tiêu</p><button type="button" onClick={() => void applyProposal()} className="mt-5 w-full rounded-xl bg-primary px-4 py-3 font-bold text-on-primary hover:bg-primary-fixed-variant">Áp dụng kế hoạch</button><p className="mt-3 text-xs text-on-surface-variant">Bạn vẫn có thể chỉnh sửa toàn bộ plan ở tab Kế hoạch & Ngân sách.</p></> : <p className="mt-3 text-sm text-on-surface-variant">Kavi sẽ tạo đề xuất sau khi đã hiểu đủ tình hình, thói quen và mục tiêu của bạn.</p>}
      </aside>
    </div>
  );
}

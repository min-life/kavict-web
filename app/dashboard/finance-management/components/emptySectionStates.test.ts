...
describe("empty finance-plan sections", () => {
  it.each([
    ["IncomePlanSection.tsx", "progress.length === 0"],
    ["ObjectivesSection.tsx", "displayObjectives.length === 0"],
  ])("mutes the complete %s surface when no records exist", (fileName, emptyCondition) => {

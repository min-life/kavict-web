...
const TABS = [
  { id: "entry", label: "Nhập liệu", icon: "edit_note", iconClass: "bg-[#e8f3ff] text-[#1877f2]" },
  { id: "plan", label: "Kế hoạch & Ngân sách", icon: "account_balance_wallet", iconClass: "bg-[#e4fbf8] text-[#0bb8ac]" },
  { id: "reports", label: "Thống kê và Báo cáo", icon: "bar_chart", iconClass: "bg-[#e7f1ff] text-[#17b6ff]" },
  { id: "advisor", label: "Kavi Advisor", icon: "smart_toy", iconClass: "bg-gradient-to-br from-[#eef4ff] via-white to-[#f8ebff] text-[#6d28d9]" },
] as const;
...
export default function FinanceWorkspace() {
  ...
  return <div className="min-h-[calc(100vh-80px)] space-y-6"><header><h1 className="text-2xl font-bold text-on-surface sm:text-3xl">Quản lý tài chính</h1><p className="mt-2 text-on-surface-variant">Chủ động ghi nhận, lập ngân sách và nhận tư vấn phù hợp với bạn.</p></header>{alerts.filter((item) => !dismissedAlerts.includes(item.category)).map((item) => <div key={item.category} className={`flex items-center gap-3 rounded-xl border p-4 ${item.status === "over-budget" ? "border-error/30 bg-error/10" : "border-orange-500/30 bg-orange-500/10"}`}><span className="material-symbols-outlined text-error">warning</span><p className="flex-1 text-sm text-on-surface">{item.status === "over-budget" ? `${item.category} đã vượt ngân sách` : `${item.category} sắp chạm hạn mức`} ({item.percent}%).</p><button onClick={() => setDismissedAlerts((current) => [...current, item.category])} className="text-sm font-bold text-on-surface-variant">Đóng</button></div>)}<div role="tablist" aria-label="Quản lý tài chính" className="grid grid-cols-2 gap-3 rounded-[28px] border border-[#dfe8f4] bg-white p-3 shadow-[0_12px_34px_rgba(15,35,70,0.10)] lg:grid-cols-4">{TABS.map((tab) => <button key={tab.id} role="tab" aria-selected={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex min-h-28 items-center justify-center gap-3 rounded-[22px] border px-4 py-4 text-left text-sm font-extrabold transition-all duration-200 sm:justify-start sm:text-base ${activeTab === tab.id ? "bg-gradient-to-br from-[#0f62fe] via-[#155eef] to-[#004ee6] text-white shadow-[0_10px_24px_rgba(0,78,230,0.28)] after:absolute after:-bottom-[9px] after:left-1/2 after:-translate-x-1/2 after:border-x-[10px] after:border-t-[9px] after:border-x-transparent after:border-t-[#004ee6]" : tab.id === "advisor" ? "border-[#73c9ff]/80 bg-gradient-to-r from-[#f8fbff] via-white to-[#fff8ff] text-[#5523be] shadow-[inset_0_0_0_1px_rgba(233,178,255,0.75)] hover:-translate-y-0.5 hover:shadow-md" : "border-transparent bg-white text-[#17213a] hover:-translate-y-0.5 hover:bg-[#f7faff] hover:shadow-sm"}`}><span aria-hidden="true" className={`flex h-20 w-20 shrink-0 items-center justify-center rounded-[22px] shadow-[0_7px_15px_rgba(21,94,239,0.12)] ${activeTab === tab.id ? "bg-white/15 text-white ring-1 ring-white/20" : tab.iconClass}`}><span className="material-symbols-outlined text-[44px]">{tab.icon}</span></span><span className="leading-snug">{tab.label}</span></button>)}</div><div role="tabpanel">{activeTab === "entry" && <TransactionEntryTab userId={user.uid} plan={plan} transactions={transactions} onSaved={handleSaved} />}{activeTab === "plan" && <PlanBudgetTab userId={user.uid} plan={plan} transactions={transactions} onSaved={handleSaved} onOpenAdvisor={openAdvisor} />}{activeTab === "reports" && <ReportsTab plan={plan} transactions={transactions} />}{activeTab === "advisor" && <KaviAdvisorTab userId={user.uid} plan={plan} transactions={transactions} onPlanApplied={applyPlan} launchRequest={advisorLaunchRequest} onLaunchRequestConsumed={consumeAdvisorLaunchRequest} />}</div></div>;
}
